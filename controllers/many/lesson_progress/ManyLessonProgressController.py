...
from plugins.many.lesson_progress.ManyLessonProgress import Many_Lesson_Progress
...
class Many_Lesson_Progress_Controller:
    ...
    def start( self ):
        for x in range( 1 ):
            ...
            many_lesson_progress_func = Many_Lesson_Progress( new_normal_register_driver )

            #動画一覧ページに移動して、そのリンクの要素を取得
            many_lesson_progress_func.go_to_lesson_list_page()

            #各動画の完了ボタンをクリックして、進捗を進めていく
            many_lesson_progress_func.progress_each_lesson()
