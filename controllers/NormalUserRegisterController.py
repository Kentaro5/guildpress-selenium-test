...
class Normal_User_Register_Controller:
	...
	def start( self ):
		...
		new_link = self.user_register_setting_func.get_new_register_page_link()
		...
		normal_register_func.quit_browser()

		self.go_to_user_list_page()

		# 登録したユーザーの削除
		self.user_register_setting_func.delete_registered_user()
	...
	def go_to_new_register_page( self ):
		...
		self.driver_action_func.single_move_admin_page( self.admin_xpath_lists['admin_new_regsiter_page_path'] )
	...
	def go_to_user_list_page( self ):
		...
		# ユーザー一覧ページへ移動
		self.driver_action_func.move_admin_page( admin_user_list_page_path, admin_user_list_sub_page_path )
