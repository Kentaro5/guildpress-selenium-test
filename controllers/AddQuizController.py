...
from plugins.addquiz.AdminAddQuiz import Admin_Add_Quiz
from plugins.addquiz.PublicAddQuiz import Public_Add_Quiz

class Add_Quiz_Controller:
	...
	def __init__(self, driver, target_url):
		...
		self.admin_add_quiz_func = Admin_Add_Quiz( driver )
	...
	def start( self ):
		...
		#クイズを設定していく
		self.admin_add_quiz_func.set_quiz_to_page()
		...
		#クイズのチェック
		self.check_quiz_page_by_normal_user()

		#トップページへ移動
		self.go_to_admin_top_page()


	def check_quiz_page_by_normal_user( self ):
		...
		public_add_quiz_func = Public_Add_Quiz( new_normal_register_driver )

		public_add_quiz_func.go_to_lesson_list_page()
		public_add_quiz_func.go_to_element_lesson_list_page()
